    ...
    >
      ...
      <a-row class="line-container">
        ...
        <div class="ticket-price display-flex-center">
          ...
          {{ $t('ticketval') }}:<span class="line-text"
            >{{ price }} {{ $t('yuan') }}
          </span>
        </div>
      </a-row>
  ...
  methods: {
    init() {
      ...
      this.lineMap.event.on('clickStation', async e => {
        ...
        if (this.store.arrivalSite) {
          ...
          this.price = ticketData.find(item => {
      ...
      if (this.$route.query && !window.$.isEmptyObject(this.$route.query)) {
        ...
        this.updateTicketMap({ departSite: this.currentSite });
      }
    ...
    onSiteChanged({
      ...
      if (type === 'price') {
        ...
        this.updateTicketMap({ departSite, arrivalSite });
        return;
      }
      this.siteChangeFn(
        type,
     ...
     */
    async updateTicketMap({ departSite = '', arrivalSite = '' }) {
