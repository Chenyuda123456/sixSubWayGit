<template>
  <div>
    <div class="stepPayresult display-flex-center van-row--align-center">
      <div>
        <!--找零-->
        ...
        <div
          v-if="cardResult?.errorList?.length && cardResult.sts === 999"
          class="pay-ticketing"
        >
          <div class="pay-result display-flex-center van-row--align-center">
            <img src="@/assets/icon_update_error.png" />
            <div class="text-box">
              <div>
                {{
                  cardResult.errorList &&
                  cardResult.errorList.length &&
                  cardResult.errorList[cardResult.errorList.length - 1][
                    'Descript' + lang
                  ]
                }}
              </div>
        ...
        <div v-else>
          ...
          <div v-if="cardResult.sts == 999 && cardResult.substs == 256">
            <div class="display-flex-center van-row--align-center">
              <img src="@/assets/icon_update_success.png" />
              <div class="text-box">
                <!--余额支付-->
                <div v-if="cardResult.paymentType == payMethods['remainder']">
                  {{
                    cardResult.mediumType == 0
                      ? $t('CardWritingSuccessful')
                      : $t('paymentSuccess')
                  }}
                </div>
                <div v-else>{{ $t('paymentSuccess') }}</div>
                <div>
        ...
        <div
          v-if="
            cardResult.processInfo.amount > 0 &&
      ...
      <!--    <div class="hint" v-if="  cardResult.mediumType == 0">
